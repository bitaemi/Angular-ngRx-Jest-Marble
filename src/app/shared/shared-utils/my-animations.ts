...
export const fadeAnimation =
    // specify animation name
    trigger("routeAnimations", [
        // mention states between which animation is executed e.g. /login <=> /signup
        transition("* <=> *", [
            // events on which to create animation
            query(":enter, :leave",
                style({ position: 'absolute', width: '98%' }),
                { optional: true }),
            // group queries to execute more than one animation 
            group([
                // enter animation is applied for the incoming page (for next state)
                query(":enter", [
                    style({ opacity: 0 }),
                    animate("0.6s", style({ opacity: '1' }))
                ], { optional: true }),
                // leave animation is applied for the page we are going to leave (current router state)
                query(":leave", [
                    style({ opacity: 1 }),
            ...
            group([
                ...
                query(":enter", [
                    animate("1s", keyframes([
                        ...
                        // at 70% of the transformation duration the dimention is 50% and the horizontal position is 30% from original position
